...
async function main() {
  ...
  const artifact = await utils.S3.loadJSON(
    S3,
    config.build.artifact.bucket,
    config.build.artifact.key,
  );
  ...
  logger.info(
    'Our contract address:%s, owner:%s, participant: %s',
    our.address,
    ourOwner,
    config.participant.our,
  );
  logger.info(
    'Their contract address:%s, owner:%s, participant: %s',
    their.address,
    theirOwner,
    config.participant.their,
  );
